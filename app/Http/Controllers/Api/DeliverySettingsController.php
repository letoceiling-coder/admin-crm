...
{
    /**
     * Получить настройки доставки для текущего пользователя и магазина (админка)
     * 
    ...
    public function index(Request $request): JsonResponse
    {
            ...
            // Если shop_id не передан, пытаемся получить из запроса
            if (!$shopId) {
                $shopId = $request->header('X-Shop-Id') ?? $request->input('shop_id');
            }
            ...
            $settings = DeliverySetting::getSettings($user->id, $shopId);
    ...
    /**
     * Обновить настройки доставки (админка)
     * 
    ...
    /**
     * Получить настройки доставки для магазина (публичный для фронтенда)
     * 
     * @param Request $request
     * @return JsonResponse
     */
    public function getSettings(Request $request): JsonResponse
    {
        ...
        // Получаем настройки для магазина (создаем по умолчанию если нет)
        $settings = DeliverySetting::getSettings(null, $shopId);
    ...
    public function calculateCost(Request $request): JsonResponse
    {
        $request->validate([
            ...
            'shop_id.required' => 'shop_id обязателен',
    ...
    public function getAddressSuggestions(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $city = $request->input('city', $settings->default_city ?? 'Екатеринбург');
